...
class request extends base
{
    ...
    public function get_bag($request)
    {
        ...
        $bag = commons::get_bag($this->user_db, array(
            'org_field_id',
            'desc'
        ));
    ...
    private function update_user($field, $item, &$now)
    {
        ...
        $result = $this->user_db->insert(array(
            'table' => 'bag',
            'values' => $item
        ));
        if (! $result) {
    ...
    private function get_searched_item()
    {
        ...
        foreach ($sch_data as &$item) {
            $item['coefficient'] = ((100 - $item['scarcity']) * 10) + ((100 - $item['size'])) - ($item['holy']) - ($item['wicked']);
    ...
    {
        $sch_pt = $this->user_db->select(array(
            'table' => 'sch_pt'
        ));
